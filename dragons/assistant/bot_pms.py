...
from dragons import Config, drgub
...
async def send_flood_alert(user_) -> None:
    ...
    if found:
        ...
        if not fa_id:
            return
        try:
            msg_ = await drgub.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await drgub.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
...
@drgub.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
...
@drgub.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@drgub.bot_cmd(incoming=True, func=lambda e: e.is_private)
@drgub.bot_cmd(edited=True, func=lambda e: e.is_private)
